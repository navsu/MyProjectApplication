...
import com.memandis.remote.utils.serviceutils.suspendinglyLoadBitmapFromUri
...
class RegisterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val emailLiveData: MutableLiveData<String?> = MutableLiveData(null)
    val isNewEmailStatusLiveData: MutableLiveData<Boolean?> = MutableLiveData(null)
    val password1: MutableLiveData<String?> = MutableLiveData( DEFAULT_EMPTY_FIELD)

    val password1Status: LiveData<PasswordValidity> =
            Transformations.map(password1) { return@map validatePasswordFormat(it) }
    val password2: MutableLiveData<String?> = MutableLiveData( DEFAULT_EMPTY_FIELD)
    ...
    private val arePasswordsIdentical: LiveData<Boolean> by lazy {
        zipLiveData(password1, password2) { pass: String?, confirmPass: String? ->
    ...
    private val arePasswordsHasCorrectFormat: LiveData<Boolean> by lazy {
        zipLiveData(password1Status,  password2Status) { a, b -> a == PasswordValidity.VALID_FORMAT
                && b == PasswordValidity.VALID_FORMAT
        }
    }

    val arePasswordsOkayToSignUp: LiveData<Boolean> by lazy {
        zipLiveData(arePasswordsIdentical, arePasswordsHasCorrectFormat) { a, b -> a && b }
    }
    ...
    fun saveCustomizationData(context: Context): Single<List<Boolean>>? {
        return profileImageUri.value?.let {
            DiaryRepo.updateProfileInfo(context, it, username.value!!)
        }
    }
