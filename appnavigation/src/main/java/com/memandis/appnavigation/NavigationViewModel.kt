...
class NavigationViewModel() : ViewModel() {
    ...
    fun onSomethingInterestingHappen() {
        ...
        val event = if (dirtyFieldForDemo) NavEvent.InterestingMainEvent
                    else NavEvent.NotInterestingMainEvent
        eventsFlow.value = event
        dirtyFieldForDemo = !dirtyFieldForDemo
    }

    /**
     * Get a single event LiveData which only received the specified event types.
     * Once the event it read it is consumed and replaced again for Empty.
     */
    @ExperimentalCoroutinesApi
