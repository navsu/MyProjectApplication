...
object FirebaseUserRepository {
    ...
    fun createUserAccount(email: String, password: String):
            Single<FirebaseUser> {
        val function: (emitter: FlowableEmitter<FirebaseUser>) -> Unit = {
                flowable ->
        ...
        return Flowable
                .create<FirebaseUser>(function, BackpressureStrategy.BUFFER)
                .singleOrError()
    }
    ...
    fun loginUserAccount(email: String, password: String): Single<FirebaseUser> {
        return Flowable.create<FirebaseUser>({
                flowable ->
    ...
    fun validateExistingUsername(username: String): Single<Boolean> {
        return Flowable.create<Boolean>({
                flowable ->
                                     val a = FirebaseDatabase.getInstance().reference
    ...
    private fun attachUserToStorageRepo() {
        ...
        Log.d(
            LOG_TAG, "Injected Firebase User reference to" +
                    " the Storage Repo -> ${firebaseUser!!.uid}")
    }
    ...
    fun attachDefaultUserToFirebaseRepositories(s: String) {
        FirebaseAppRepository.setUserDatabaseReference(s)
        Log.d(LOG_TAG, "Injected Firebase User reference to the App Repo ->" +
                " ${s}")
        FirebaseStorageRepository.setUserStorageReference(s)
    ...
    fun UserRemote.email() = firebaseUser?.email!!

    fun logoutUserAccount() {
        Log.d(
            LOG_TAG, "Logging out user -> " +
                "uid={${firebaseAuth.currentUser!!.uid}} from Firebase Auth")
